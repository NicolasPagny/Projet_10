...
def _pred_svds(clicks: pd.DataFrame, n_factors:int = 5):
    ...
    predicted_rating_norm_df = pd.DataFrame(predicted_rating_norm, columns=clicks_articles_matrix.columns, index=clicks_articles_matrix.index)
...
def most_popular_articles(clicks: pd.DataFrame, n: int):
...
def cosine_similar_articles_per_user(user_id:int, clicks: pd.DataFrame, articles_embeddings: pd.DataFrame):
    """Obtenir les articles les plus similaires pour chaque utilisateur
    ...
    # Obtenir les 5 articles les plus similaires pour l'utilisateur
...
def best_cosine_similar_articles_per_user(user_id: int, clicks: pd.DataFrame, articles_embeddings: pd.DataFrame, n: int = 5):
    ...
    article_id = _last_clicked_articles_user(id, clicks)["click_article_id"]
...
def preds_svds_user(user_id: int, clicks: pd.DataFrame, n_factors:int = 5):
    ...
    return predicted_rating_norm.loc[user_id]

def best_preds_svds_users(users_id: int, clicks: pd.DataFrame, min_clicks: int = 2, n_factors:int = 5, n: int = 5):
    ...
    # Obtenir les meilleures prédictions pour les utilisateurs
    best_preds_users = {}
    for user_id in users_id:
        if user_id in predicted_rating_norm.index:
            if n is not None:   
                user_preds = predicted_rating_norm.loc[user_id].nlargest(n)
            else:
                user_preds = predicted_rating_norm.loc[user_id]
            best_preds_users[user_id] = user_preds

    return best_preds_users

def hybrid_recommander(users_id: int, clicks: pd.DataFrame, articles_embeddings: pd.DataFrame, n_factors:int = 15):
    ...
    content_based_articles = cosine_similar_articles_per_user(users_id, clicks, articles_embeddings)

    # Obtenir les n articles recommandés avec la factorisation de matrice SVD
    collaborative_filtering_articles = preds_svds_users(users_id, clicks, n_factors)
    index_to_keep = list(collaborative_filtering_articles.keys())
    print(index_to_keep)
    filtered_content_based_articles = {key: value for key, value in content_based_articles.items() if key in index_to_keep}


    return filtered_content_based_articles, collaborative_filtering_articles 